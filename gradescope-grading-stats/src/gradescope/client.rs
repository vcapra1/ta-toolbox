...
impl GradescopeClient {
    ...
    /// Fetch the grading stats for the specified assignment in the specified course.  The
    /// assignment must be an online assignemnt or an exam type.
    ///
    /// # Arguments
    ///
    /// * `course_id` - The course ID on [Gradsecope]
    ...
    /// [`UnexpectedResponse`]: enum.ClientError.html
    ///
    pub fn fetch_grading_stats(&self,
                               course_id: u64,
                               assignment_id: u64) -> Result<stats::Stats, ClientError> {
        /* Fetch stats */
        stats::Stats::new(course_id, assignment_id, &self.http_client)
    }

    pub fn refresh_grading_stats(&self,
                                 stats: &mut stats::Stats) -> Result<(), ClientError> {
        /* Fetch stats */
        stats.refresh(&self.http_client)
    }
}
